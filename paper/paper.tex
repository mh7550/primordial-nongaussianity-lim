...
% Float parameters to reduce white-space gaps
...
\caption{Joint forecast summary.  (a) $\sigma(f_{\rm NL}^{\rm local})$ across estimators---Planck CMB ($5.1$), SPHEREx single-tracer ($1.8$), SPHEREx multi-tracer with auto-spectrum only ($0.89$), and the full cross-power Fisher matrix ($0.71$).  The $\sigma=1$ multi-field threshold is crossed.  (b) Realistic H$\alpha$ S/N as a function of redshift under the v28 noise model.  (c) Deep-field vs.\ all-sky comparison. The all-sky survey carries $125\times$ more large-scale modes than the deep field, which dominates the $f_{\rm NL}^{\rm local}$ forecast.  (d) Per-line contributions to the single-tracer $\sigma(f_{\rm NL}^{\rm local})$ forecast, with the full multi-tracer constraint shown for comparison.}
\label{fig:joint}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figure_3_noise_model.pdf}
