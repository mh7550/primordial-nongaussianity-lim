...
\begin{abstract}
The amplitude of local primordial non-Gaussianity $f_{\rm NL}^{\rm local}$ is a fingerprint of inflationary physics: sub-unity sensitivity discriminates single-field models, which predict $|f_{\rm NL}^{\rm local}| \ll 1$, from broad classes of multi-field scenarios.  We construct a complete forward pipeline that propagates a $\Lambda$CDM cosmology through the line-intensity-mapping (LIM) signal model into the full $92\times 92$ angular power spectrum matrix $C_\ell^{\nu\nu'}$, and combine it with a multi-tracer Fisher forecast following the prescription of \citet{Seljak2009} and the LIM framework of \citet{Cheng2024}.  We introduce three methodological contributions relative to previous SPHEREx forecasts: (i) the full $92\times 92$ cross-power matrix evaluated with channel-adaptive Limber and Bessel integration; (ii) the wavelength-dependent SPHEREx v28 noise model in place of the constant-noise approximation used in prior work; and (iii) a realistic sky fraction $f_{\rm sky}=0.60$ that accounts for galactic-plane masking.  We find $\sigma(f_{\rm NL}^{\rm local}) = 0.71$, a factor of $7.2$ improvement over the Planck 2018 limit of $5.1$.  The multi-tracer auto-spectrum already provides a $2.8\times$ improvement over Planck; the off-diagonal cross-power contributes a further $20\%$; and modes with $\ell < 50$ carry roughly $40\%$ of the constraining power, confirming that the $k^{-2}$ scale-dependent bias dominates the signal.  SPHEREx therefore crosses the $\sigma(f_{\rm NL}^{\rm local}) = 1$ threshold, entering the regime that discriminates between broad classes of inflationary models.
\end{abstract}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{joint_summary_updated.png}
\caption{Joint forecast summary.  (a) $\sigma(f_{\rm NL}^{\rm local})$ across estimators---Planck CMB ($5.1$), SPHEREx single-tracer ($1.8$), SPHEREx multi-tracer with auto-spectrum only ($0.89$), and the full cross-power Fisher matrix ($0.71$).  The $\sigma=1$ multi-field threshold is crossed.  (b) Realistic H$\alpha$ S/N as a function of redshift under the v28 noise model.  (c) Deep-field vs.\ all-sky comparison showing the $125\times$ mode advantage that makes the all-sky survey decisively better for $f_{\rm NL}^{\rm local}$.  (d) Per-component accounting of the methodological changes implemented relative to a constant-noise, diagonal-only baseline.}
\label{fig:joint}
\end{figure*}

The remainder of the paper is organised as follows.  Section~\ref{sec:theory} reviews the scale-dependent bias, the LIM angular power spectrum, and the multi-tracer Fisher matrix.  Section~\ref{sec:pipeline} describes the pipeline implementation, organised into the five phases that build from the cosmological kernel to the realistic Fisher forecast.  Section~\ref{sec:results} presents the headline result and its decomposition.  Section~\ref{sec:discussion} compares to previous work and identifies open systematics.  Section~\ref{sec:conclusions} concludes.
...
with
\begin{equation}
\Delta b(k,z) = 3 f_{\rm NL}^{\rm local}\,\bigl[b_1(z)-1\bigr]\,\delta_c \,\frac{\Omega_m H_0^2}{c^2\, k^2\, T(k)\, D(z)},
\label{eq:bias-delta}
\end{equation}
...
\subsection{Baseline single-tracer Fisher forecast}
...
\subsection{LIM signal model and full \texorpdfstring{$C_\ell$}{Cl} matrix}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{lim_combined_figure2.png}
...
\subsection{Bayesian inference scaffolding}
...
\label{eq:headline}
\end{equation}
This represents a factor of $7.2$ improvement over the Planck 2018 bound \citep{Planck2020PNG}.  It is below the $\sigma=1$ threshold by a margin that survives plausible variation in the systematics enumerated in Section~\ref{sec:discussion}.
...
\item SPHEREx single-tracer (best single line, $\ell_{\rm min}=2$, $f_{\rm sky}=0.60$): $\sigma(f_{\rm NL}^{\rm local}) = 1.8$, a factor of $2.8$ over Planck.
\item SPHEREx multi-tracer, diagonal-only Fisher: $\sigma(f_{\rm NL}^{\rm local}) = 0.89$, a further factor of $2$ over the single-tracer case.
\item SPHEREx multi-tracer, full cross-power: $\sigma(f_{\rm NL}^{\rm local}) = 0.71$, a further $20\%$ improvement.
\end{itemize}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{fix7_cross_power_full_vs_diagonal.png}
\caption{Full $92\times 92$ cross-power Fisher matrix compared to the diagonal-only approximation.  Top: visualization of $\Sigma_\ell$ at a representative multipole---diagonal-only (left) and full matrix (right), showing concentrated off-diagonal weight at line pairs observed at matched physical redshifts.  Bottom: the resulting $\sigma(f_{\rm NL}^{\rm local})$ improves from $0.89$ to $0.71$, a $20\%$ gain consistent with the \citet{Seljak2009} cosmic-variance cancellation operating at the lower end of its typical $20$--$40\%$ range due to the localised structure of the off-diagonal terms.}
