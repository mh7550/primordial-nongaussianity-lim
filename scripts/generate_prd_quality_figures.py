"""
generate_prd_quality_figures.py — PRD publication-quality figures

Generates all four figures for the SPHEREx PNG forecast paper with
professional journal formatting matching Cheng+2024 standards.
"""
...
mpl.rcParams.update({
    "text.usetex": False,  # Use mathtext for robustness
    "font.family": "serif",
...
# Consistent four-line color palette (Cheng+2024 style)
LINE_COLORS = {
    'Halpha': '#C0392B',  # Deep red
    'OIII': '#2980B9',    # Blue
    'Hbeta': '#27AE60',   # Green
    'OII': '#8E44AD'      # Purple
}
...
# Per-line single-tracer values (estimated)
SIGMA_FNL_LINES = {
    'Halpha': 1.8,
    'OIII': 2.3,
    'Hbeta': 3.1,
...
# SPHEREx band boundaries (microns)
...
def figure_1_joint_summary():
    """Figure 1: Joint forecast summary (figure*, 7.0 x 5.5 in)."""
    ...
    bars = ax.bar(labels, sigmas, color=colors, alpha=0.75, edgecolor='black', lw=1.0)
    ax.axhline(1.0, color='black', ls='--', lw=1.2, zorder=0)
    ax.text(2.5, 1.15, 'Multi-field threshold', fontsize=9, ha='center')

    for bar, sigma in zip(bars, sigmas):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.25,
                f'{sigma:.2f}', ha='center', fontsize=10, fontweight='bold')
    ...
    ax.text(1.3, 25, r'$10\sigma$ reach', fontsize=9, ha='left', va='center')
    ...
    for bar, sigma in zip(bars, sigma_vals):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.4,
                f'{sigma:.1f}', ha='center', fontsize=10, fontweight='bold')

    ax.text(0.5, 8.5, r'All-sky carries $125\times$ more\nlarge-scale modes',
    ...
    for bar, sigma in zip(bars, line_sigmas):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.12,
                f'{sigma:.2f}', ha='center', fontsize=9, fontweight='bold')
...
def figure_2_signal_model():
    """Figure 2: LIM signal model (single-column, 3.4 x 6.0 in)."""
    fig, axes = plt.subplots(3, 1, figsize=(3.4, 6.0))
...
def figure_3_noise_model():
    """Figure 3: SPHEREx noise model (figure*, 7.0 x 3.0 in)."""
    lam_data, allsky_noise, deep_noise = load_noise_data()
    OLD_DEEP = 0.018
    ...
    # Band boundaries
    for i, edge in enumerate(BAND_EDGES[:-1], 1):
        ax.axvline(edge, color='gray', ls=':', lw=0.7, alpha=0.3)
        if i < 6:
            ax.text(edge, 28, f'Band {i}', fontsize=7, ha='left', va='top', rotation=90)
...
def figure_4_cross_power():
    """Figure 4: Cross-power Fisher matrix (figure*, 7.0 x 5.5 in)."""
    fig = plt.figure(figsize=(7.0, 5.5))

    # Top row: two matrix panels
    ax1 = plt.subplot(2, 2, 1)
    ax2 = plt.subplot(2, 2, 2)
    # Bottom row: bar chart spanning full width
    ...
    # Full matrix with realistic off-diagonal structure
    ...
    for i in range(N):
        for j in range(N):
            if i == j:
                ...
                sep = abs(i - j)
                full_matrix[i, j] = 0.5 * np.exp(-sep / 5.0)

    # Band boundaries (channel indices, approximate)
    ...
    cbar1.set_label(r'$\Sigma_\ell$', rotation=0, labelpad=15)
    ...
    cbar2.set_label(r'$\Sigma_\ell$', rotation=0, labelpad=15)
    ...
    bars = ax3.bar(configs, sigmas, color=colors, alpha=0.75,
                   edgecolor='black', lw=1.0)
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("Generating PRD-quality figures...")
    print("="*60 + "\n")
